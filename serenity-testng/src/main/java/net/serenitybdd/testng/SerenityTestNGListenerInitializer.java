...
public class SerenityTestNGListenerInitializer {
     ...
     */
    public SerenityTestNGListenerInitializer(final Class<?> klass) throws InitializationError {
     ...
     * @throws InitializationError if some JUnit-related initialization problem occurred
     */
    public SerenityTestNGListenerInitializer(Class<?> klass, com.google.inject.Module module) throws InitializationError {
        this(klass, Injectors.getInjector(module));
    }

    public SerenityTestNGListenerInitializer(final Class<?> klass,
    ...
    public SerenityTestNGListenerInitializer(final Class<?> klass, final BatchManager batchManager) throws InitializationError {
        this(klass,
                ThucydidesWebDriverSupport.getWebdriverManager(),
...
//                StepEventBus.getEventBus().exampleFinished();
    ...
    protected JUnitStepListener initListenersUsing(final Pages pageFactory) {
        ...
        return JUnitStepListener.withOutputDirectory(getConfiguration().getOutputDirectory())
                ...
                .and().withTestClass(getTestClass().getRealClass())
                .and().build();
    }
    ...
    private boolean webtestsAreSupported() {
        return TestCaseAnnotations.supportsWebTests(this.getTestClass().getRealClass());
    }
   ...
   /*
    ...
    protected void runChild(FrameworkMethod method, RunNotifier notifier) {
        ...
        if (failureDetectingStepListener.lastTestFailed() && maxRetries() > 0) {
            retryAtMost(maxRetries(), new SerenityRunner.RerunSerenityTest(method, notifier));
        }
    }*/

    /*private void retryAtMost(int remainingTries,
                             SerenityRunner.RerunTest rerunTest) {
    ...
    /*class RerunSerenityTest implements SerenityRunner.RerunTest {
    ...
    private boolean shouldSkipTest(FrameworkMethod method) {
        return !tagScanner.shouldRunMethod(getTestClass().getRealClass(), method.getName());
    }
    ...
    private void testStarted(FrameworkMethod method) {
        //TODO
        getStepListener().testStarted(Description.createTestDescription(method.getMethod().getDeclaringClass(),"blabla" /*testName(method)*/));
    }
